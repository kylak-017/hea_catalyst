...
for _ in range(5):
    n += 1
    #make supercell
    supercell = structure.make_supercell(2)
    print(supercell)


    all_zr = [i for i, s in enumerate(supercell) if s.species_string == "Zr"]

    metals = ["Zr", "Hf", "Ti", "Pd", "Ru"]
    n_zr = len(all_zr)
    n_metals = len(metals)
    atoms_per_metal = n_zr // n_metals
    remainder = n_zr % n_metals

    # Create the equiatomic list
    metal_list = metals * atoms_per_metal + metals[:remainder]
    random.shuffle(metal_list)

    for idx, i in enumerate(all_zr):
        supercell[i] = metal_list[idx] # a random metal site is subsituted for Zr

    print(supercell)
    print(supercell.composition)

    supercell.to(fmt="cif", filename=f"doped_UiO66{n}.cif")
